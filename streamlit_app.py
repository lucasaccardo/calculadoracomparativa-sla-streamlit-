...
import hashlib
import os
import base64 # Importa base64 para codificar a imagem

# --- CONFIGURAÇÃO DA PÁGINA ---
st.set_page_config(
    page_title="Calculadora SLA | Vamos",
    page_icon="logo_sidebar.png" if os.path.exists("logo_sidebar.png") else "🚛", # Ajuste para PNG
    layout="wide",
...
def aplicar_estilos():
    try:
        ...
        st.markdown(
            ...
            <style>
            .stApp {{
                background-image: url(data:image/png;base64,{bg_image_base64}); /* Ajuste para PNG */
        ...
        pass # Se o arquivo não for encontrado, o fundo não é aplicado
...
def load_user_db():
    try:
        ...
        if "accepted_terms_on" not in df.columns: df["accepted_terms_on"] = None # Garante coluna para termos
        return df
    except (FileNotFoundError, pd.errors.EmptyDataError):
        admin_user = {
            "username": ["lucas.sureira"], "password": [hash_password("Brasil@@2609")], "role": ["admin"],
            "full_name": ["Administrador Principal"], "matricula": ["000000"], "accepted_terms_on": [None]
        }
...
def calcular_sla_simples(data_entrada, data_saida, prazo_sla, valor_mensalidade, feriados=0):
    ...
    if dias_uteis <= prazo_sla:
        ...
        status = "Fora do SLA"
        dias_excedente = dias_uteis - prazo_sla # CORREÇÃO APLICADA AQUI
...
def renderizar_sidebar():
    with st.sidebar:
        try: st.image("logo_sidebar.png", width=100) # Ajuste para PNG
        except FileNotFoundError: pass # Não mostra imagem se não encontrar
...
aplicar_estilos() # Aplica os estilos no início

df_users = load_user_db()
if st.session_state.tela == "login":
    st.markdown("<div class='login-container'>", unsafe_allow_html=True)
    st.markdown("<div class='login-logo'>", unsafe_allow_html=True)
    try: st.image("logo.png", width=300) # Ajuste para PNG
    except FileNotFoundError: st.header("🚛 Vamos Locação")
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<h1 style='text-align: center;'>Plataforma de Calculadoras SLA</h1>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    
    col1, col2, col3 = st.columns([1, 1.5, 1]) # Centraliza o conteúdo
    with col2:
    ...
    # Aqui o background padrão do Streamlit pode voltar, pois a div .main-container não envolve
    # Por isso, vamos adicionar um container para o conteúdo principal
    st.markdown("<div class='main-container'>", unsafe_allow_html=True) # Começa o container
    ...
    **4. Aceite dos Termos**
    ...
    renderizar_sidebar()
    st.markdown("<div class='main-container'>", unsafe_allow_html=True) # Começa o container principal
