...
if st.session_state.tela == "login":
    ...
    # --- SEÇÃO DE ADICIONAR CENÁRIO (FLUXO SEQUENCIAL) ---
    if st.session_state.estado_calculo == "adicionando":
        ...
        with col_form:
            with st.form(key=f"form_cenario_{len(st.session_state.cenarios)}"):
                st.subheader("1. Detalhes do Veículo e Serviço")
                st.text_input("🔍 **Digite a placa do veículo:**", key="input_placa", on_change=buscar_placa, args=(df_base,))
                ...
                if st.form_submit_button(f"➡️ Calcular Cenário {len(st.session_state.cenarios) + 1}", use_container_width=True, type="primary"):
                    if st.session_state.cliente_info:
                        ...
                        if entrada >= saida: st.error("A data de saída deve ser posterior à de entrada.")
                            ...
                            limpar_dados_parcialmente = {"pecas_atuais": [], "cliente_info": None, "input_placa": ""}
                            for key, value in limpar_dados_parcialmente.items(): st.session_state[key] = value
                            st.rerun()
